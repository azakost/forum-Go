...
func setJWT(username string, w http.ResponseWriter) {
	...
	data, marshalError := json.Marshal(jwt)
	err(marshalError)
...
func validateJWT(w http.ResponseWriter, r *http.Request) bool {
	...
	if base64Error != nil {
		// If value invalid - delete cookie, probably somebody has changed it
	...
	unmarshalError := json.Unmarshal(decrypted, &jwt)
	...
	if val != jwt.Token {
		delete(sessions, jwt.Username)
		addCookie(w, "jwt", "", time.Unix(0, 0))
		return false
	}

	// Filter 2 - If token is expired
	if time.Now().After(jwt.Expire) {
		delete(sessions, jwt.Username)
		addCookie(w, "jwt", "", time.Unix(0, 0))
		return false
	}
	...
	if jwt.Expire.Before(time.Now().Add(tokenRefresh)) {
		setJWT(jwt.Username, w)
	}
	return true
}
